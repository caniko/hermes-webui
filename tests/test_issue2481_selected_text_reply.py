...
def test_selected_text_reply_context_cards_are_built_with_text_nodes():
    ...
    assert "quote.title=_selectedContextPreview(s.text)" in js
    assert "quote.title=s.text" not in js
...
def test_selected_text_reply_styles_and_i18n_exist_for_all_locales():
    ...
    assert "function _stashUserSelectedContextBlocks" in ui
    assert "only this exact labelled blockquote shape gets custom treatment" in ui
...
def test_sent_selected_context_blocks_are_rendered_without_enabling_user_markdown():
    ...
    assert "function _sentSelectionContextBlockHtml(label, quoteText)" in ui
